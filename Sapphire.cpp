...
//======================================================================
// return userno (1 based) if OK, else negative
int Sapphire::DbLogin(const char* username) {
	// anyname?
#if DDLLIB
	if(!isname(username))
		return -1;
	dbUserNo = userAdd(username) + 1;
	return dbUserNo;
#else
	return 0;
#endif
...
//======================================================================
// return userno (1 based) if OK, else negative
int Sapphire::DbLogout(int userno) {
#if DDLLIB
	userRetire(userno - 1);
#endif
	return 0;
...
Return value
	...
	Otherwise a NULL will be returned. DbGetErrno may be called to get the
...
RDbf* Sapphire::DbUse(const char* dbfname) {
	...
	for (ldbf = dbDbfRoot; ldbf; ldbf = ldbf->GetLink()) {
		if (str_icmp(ldbf->GetName(), dbfname) == 0)
			return(0);
		}

	dbf = new RDbf(dbfname, dbUserNo);
	rc = dbf->openDbf(dbfname);
	if (rc <= 0) {
		delete dbf;
		return 0;
		}
